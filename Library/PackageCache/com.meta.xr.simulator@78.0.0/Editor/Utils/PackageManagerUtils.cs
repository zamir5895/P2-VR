...
namespace Meta.XR.Simulator.Editor
{
    internal class PackageManagerUtils
    {
        ...
        public virtual int? GetPackageVersion(string packageId)
        {
            var packageInfo = PackageList.GetPackage(packageId);

            return packageInfo != null ? Utils.GetMajorVersion(packageInfo.version) : null;
        }
    }
}
